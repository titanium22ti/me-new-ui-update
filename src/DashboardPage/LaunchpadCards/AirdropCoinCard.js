...
import { RewardDistrubutorABI, RewardDistrubutorAddress, AirDropCoinABI, AirDropCoinAddress } from '../../abi/abi';
...
const AirdropCoinCard = ({ balanceOfTokens, CampaignId }) => {
  ...
  const getUserAVAXBalance = async () => {
    ...
    const getAuctionDetails = async () => {
        try {
            const auctionContract =  new ethers.Contract(AirDropCoinAddress, AirDropCoinABI, provider);
            // let address = localStorage.getItem("walletAddress").toLowerCase()
          // Example: Get a specific property from the contract
          const getIsWhitelisted = await auctionContract._whitelist(address);
          const getRewardAmount = await auctionContract.getRewardAmount(address);
  ...
  const rewardClaim = async () => {
    try {
      ...
      const auctionContract =  new ethers.Contract(AirDropCoinAddress, AirDropCoinABI, signer);
...
export default AirdropCoinCard;