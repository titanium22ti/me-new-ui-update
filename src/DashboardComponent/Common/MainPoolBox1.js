...
import { pooladdress,deployeraddress, tokencreator, assetbalance,poolassetbalance, lpassetbalance, liability, liabilitycal, poollpbalance, createtpairhistory } from '../../config';
   ...
   const[loader, setLoader] = useState(false);
    ...
    const asset = async () => {
      ...
      if(localStorage.getItem("walletAddress")  === null || localStorage.getItem("walletAddress")  === "" || localStorage.getItem("walletAddress")  === " " ||  localStorage.getItem("walletAddress") === '' || localStorage.getItem("walletAddress") === undefined){
        ...
        setdaibalance(b2 )
        let c = await poolassetbalance(localStorage.getItem("walletAddress"),pooladdress,"USDT")
      ...
      let e = await liabilitycal("USDT")
      console.log("liability",e )
      setliability(e )

      let f = await poollpbalance("USDT")
      console.log("f",f)
      setpoollpbalance(f)
    ...
    const maxval = async() =>{
        ...
        if(clickbutton == "Deposit"){
            setamount(Math.abs(usdtbalance)/100000000)
        }
        else{
...
      }
    ...
    const DepositUSDT = async () => {
      setLoader(true)
        let a = await depositft(amount);
        if(a==1){
            return;
        }
        let b = await depositcheck(amount*100000000,usdtbalance)
        if(b==1){
            return;
        }
        ...
        const transaction = {
            ...
            type_arguments: [`${tokencreator}::TestCoins::USDT`],
        };
        try {
    ...
    const withdrawother = async () => {
      setLoader(true)
        ...
        const transaction = {
            ...
            arguments: [pooladdress, parseInt(amount*100000000), 0,  g+1000],
            type_arguments: [`${tokencreator}::TestCoins::USDT`,`${tokencreator}::TestCoins::${Tovalue2.text}`],
        };
        try {
            // const pendingTransaction = await (window).aptos.signAndSubmitTransaction(transaction);
            let pendingTransaction = await swappet(transaction)
                    ...
                    <div className='Withdraw_main_row_row'>
                        <div className='coverage_ratio_block'>
                            <div className='coverage_ratio_logo'>
                                <img src={Item.item_logo} alt="usdt" />
                                <p>{Item.item_logo_name}</p>
                            </div>
                            <p className='ratio_common_p'>Coverage Ratio: 
                            {/* <span>{liability?parseFloat(Math.abs(poolusdt/liability)).toFixed(3):"0.0"}</span>  */}
                             <span>{liability?formatter(Math.abs(poolusdt/liability)):"0.0"}</span> 
                        ...
                        <div className='deposits_block'>
                            ...
                            <h5 className='mer_text_h5'>$ {poolusdt?parseFloat(Math.abs(poolusdt/100000000)).toFixed(2):"0.0"}</h5>
                            {/* <p className='Market_text_p'>{poolusdt?parseFloat(Math.abs(poolusdt/100000000)).toFixed(2):"0.0"}USDT</p> */}
                        </div>
                        ...
                        <div className='deposits_block deposits_block_line'>
                            ...
                            <h5 className='mer_text_h5'>${poollpBalance&&poolusdt ?parseFloat(Math.abs((parseInt(poollpBalance)+parseInt(poolusdt))/100000000)).toFixed(2):"0.0"}</h5>
                            {/* <p className='Market_text_p'>{poollpBalance?parseFloat(Math.abs((parseInt(poollpBalance)+parseInt(poolusdt))/100000000)).toFixed(2):"0.0"}</p> */}
                        ...
                        <div className='deposits_block'>
                            ...
                            {/* <p className='Market_text_p'>USDT</p> */}
                        </div>
                    </div>
